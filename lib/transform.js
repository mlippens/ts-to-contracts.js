 ...
 */
var esprima, falafel, utils, moduleName, tmpl, fs, config, exec;
...
/**
 * This little function calls synchronously to typescript to compile a piece of code.
    ...
    var ModuleContract = (function () {
        ...
        ModuleContract.prototype.toContract = function() {
            ...
            for (i = 0; i < this.classes.length; i ++) {
                 ...
                 */
                defined_classname = registerVariable(classname + "_class");
                defined_instancename = registerVariable(classname);
        ...
        for (i = 0; i < body.length; i++ ) {
            ...
            if (typeof item.typeDeclaration !== 'undefined') {
                ...
                if (declaration.type === "InterfaceDeclaration") {
                    ...
                    //todo: dont rely on source() anymore, keep the transformed "version" in the k, not on the actual ast
...
exports.toContracts = function (src, module) {
    ...
    return falafel(src, function (node) {
        ...
        if (node.type === "InterfaceDeclaration") {
            ...
            //todo: dont actually transform if inside module
        ...
        if (node.type === "ModuleDeclaration" && typeof node.ambient !== 'undefined' && node.ambient) {
            var moduleK, result, defined_classname;
            moduleK = utils.processModule(node);
            result = moduleK.toContract();
            if (! moduleK.external) {
                // in this case we just register it, and don't export.
                defined_classname = utils.registerVariable(moduleK.moduleName);
                result = result + utils.defineVariable(defined_classname, moduleK.moduleContract.toContract());
            }
            node.update(result.addSemiColon());
        }

        /* regular modules */
        if (node.type === "ModuleDeclaration" && (typeof node.ambient === "undefined" || !node.ambient)) {4
            var objectContract;
            ...
            compile(node.source(), function(out) {
        ...
        if (node.type === "ClassDeclaration" && typeof node.ambient !== 'undefined' && node.ambient) {
            ...
            if (! node.parent.type === "ModuleMember") {
