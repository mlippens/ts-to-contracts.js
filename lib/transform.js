...
utils = (function (utils) {
    ...
    var doubleFun = function(newHandler, callHandler, opt) {
        if (typeof opt === 'undefined') {
            opt = {};
        }
        return templatify("doublefun",
            {"newdom": newHandler.dom, "newrng": newHandler.rng,
            "calldom": callHandler.dom, "callrng": callHandler.rng});
    };
	...
	var processObject = function(node) {
		...
		for (i = 0; i < node.body.length; i++) {
			if (typeof node.body[i].functionType !== 'undefined') {
				callType = higherorderParams(node.object[i].functionType);
			} else if (node.body[i].key.name === 'constructor') {
                newType = higherorderParams(node.object[i].value);
            } else {
				key = node.body[i].key;
