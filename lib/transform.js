...
utils = (function (utils) {
    ...
    var processModule = function (node) {
        var keys, types, body, i, item, key;
        keys = [];
        types = [];
        body = node.body;
        ...
        return new ObjectContract(keys, types);
    }
    ...
    return falafel(src, function (node) {
        ...
        if (node.type === "AmbientFunctionDeclaration") {
            ...
            if (declaration.type === 'FunctionTypeDeclaration') {
                node.update(config.toplevel_prefix + identifier + "=" + utils.guard(type, identifier));
            }
        }

        if (node.type === "ModuleDeclaration") {
            var objectContract, identifier;
            identifier = node.id.name;
            objectContract = utils.processModule(node);
            node.update(config.toplevel_prefix + identifier + "=" + utils.guard(objectContract.toContract(), identifier));
        }
