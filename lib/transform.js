...
utils = (function (utils) {
    ...
    var ModuleContract = (function () {
        var ModuleContract =  function(objContract) {
        ...
        ModuleContract.prototype.toContract = function() {
            var i, result;
            result = "";
            for (i = 0; this.interfaces.length; i ++) {
                result = result + tmpl("declaration", {"name": "foo", "value": this.interfaces[i].toContract()});
            }
            result = result + tmpl("exportdeclaration", {"name": "foo", "value": this.moduleContract.toContract()});
            return result;
        }
    ...
    var processModule = function (node) {
        ...
        module = new ModuleContract(object);
        keys = object.keys;
        types = object.values;
        body = node.body;
        ...
        for (i = 0; i < body.length; i++ ) {
            ...
            if (typeof item.typeDeclaration !== 'undefined') {
                ...
                if (declaration.type === 'InterfaceDeclaration') {
                    module.addInterface(processInterface(declaration));
                } else {
                    keys.push(key);
        ...
        return new ModuleContract(object);
    }
...
exports.toContracts = function (src, module) {
    ...
    return falafel(src, function (node) {
        ...
        if (node.type === "ModuleDeclaration") {
            ...
            guarded = utils.guard(objectContract.toContract(), identifier);
            node.update(tmpl("exportdeclaration", {"name": identifier, "value": guarded}));
        }
        ...
        if (node.type === "ClassDeclaration" && typeof node.ambient !== 'undefined' && node.ambient) {
            ...
            var elements, superclass, constructor, key, value, staticMembers, instanceMembers, classContract, className;
            ...
            //to test: subclasses: although this should be no problem according to todocontracts.coffee project
            ...
            instanceMembers = utils.processInterface(instanceMembers).toContract();
            staticMembers = utils.processInterface(staticMembers);
            staticMembers.keys.push({name: "prototype"});
            staticMembers.values.push(instanceMembers);
            staticMembers.newType = constructor;
